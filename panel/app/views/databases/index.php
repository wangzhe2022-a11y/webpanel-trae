...
<div class="panel-card">
    ...
    <table class="layui-table" style="margin:0">
        ...
        <tbody>
        <?php if (!$databases): ?>
            <tr><td colspan="6" style="text-align:center;color:#999;padding:40px">暂无数据库</td></tr>
        <?php endif; ?>
...
<form style="padding:20px 20px 0" id="createDbForm">
    ...
    <div style="color:#999;font-size:12px;padding-left:110px">
        主机地址：<b>localhost</b>（WordPress 安装在本机时填写）；密码自动生成。
    </div>
</form>
</script>
...
layui.use(['layer'], function () {
    ...
    function showSecret(title, name, user, pass) {
        layer.open({
    ...
    $('#btnCreateDb').on('click', function () {
        layer.open({
            type: 1, title: '创建数据库', area: ['520px', '360px'],
            ...
            yes: function (idx, layero) {
                ...
                WP.post('/databases/create', {
                    ...
                    layer.close(idx);
                    showSecret('创建成功', res.name, res.username, res.password);
                });
            }
        });
    });
